...
struct LevelStackEntry
{
	...
	//Y轴体素下标
	int y;
	//对应rcCompactHeightfield spans数组的下标
	int index;
};
}  // namespace

//计算各个span到边界的距离（0表示边界） 若span不是四个方向都有可达到的span时 被视为边界 被标记为0
static void calculateDistanceField(rcCompactHeightfield& chf, unsigned short* src, unsigned short& maxDist)
{
	...
	// Mark boundary cells.
	// 若span不是四个方向都有可达到的span时 被视为边界 被标记为0
	for (int y = 0; y < h; ++y)
	{
		for (int x = 0; x < w; ++x)
		{
...
//		ctx->startTimer(RC_TIMER_DIVIDE_TO_LEVELS);
		//sId为0时 拿数据 1-7处理数据
		if (sId == 0)
			//根据各个span的距离来判断lv，且与初始lv相差不超过 NB_STACKS
			sortCellsByLevel(level, chf, srcReg, NB_STACKS, lvlStacks, 1);
		else 
			//将sId-1 lv的span数据拷贝到sId lv的栈中
